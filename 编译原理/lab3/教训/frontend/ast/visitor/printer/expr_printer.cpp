...
namespace FE::AST
{
    ...
    void ASTPrinter::visit(ConditionalExpr& node, std::ostream* os)
    {
        emitHeader(*os, "ConditionalExpr");
        std::vector<ExprNode*> children;
        if (node.cond) children.push_back(node.cond);
        if (node.trueExpr) children.push_back(node.trueExpr);
        if (node.falseExpr) children.push_back(node.falseExpr);
        ...
        for (size_t i = 0; i < cnt; ++i)
        {
            ...
            if (!child) continue;
            withChild(i + 1 == cnt, [&]() { apply(*this, *child, os); });
        }
    }
}  // namespace FE::AST

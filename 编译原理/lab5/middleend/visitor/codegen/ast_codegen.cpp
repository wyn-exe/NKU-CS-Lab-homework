...
namespace ME
{
    ...
    void ASTCodeGen::handleGlobalVarDecl(FE::AST::VarDeclStmt* decls, Module* m)
    {
        ...
        {
            auto* lval = d ? dynamic_cast<FE::AST::LeftValExpr*>(d->lval) : nullptr;
            if (!lval) continue;
            auto it = glbSymbols.find(lval->entry);
            if (it == glbSymbols.end()) continue;
            auto& attr = it->second;

            DataType dt = convert(attr.type);
            if (!attr.arrayDims.empty())
            {
                m->globalVars.emplace_back(new GlbVarDeclInst(dt, lval->entry->getName(), attr));
            }
            else
            {
                ...
                {
                    auto& v = attr.initList[0];
                    if (v.type == FE::AST::floatType)
                        initOp = getImmeF32Operand(v.getFloat());
                    else
                        initOp = getImmeI32Operand(v.getInt());
                }
                m->globalVars.emplace_back(new GlbVarDeclInst(dt, lval->entry->getName(), initOp));
            }
        }
    }
    ...
    void ASTCodeGen::visit(FE::AST::Root& node, Module* m)
    {
        ...
        for (auto* s : *stmts)
        {
            if (!s) continue;
            if (s->isVarDeclStmt())
                handleGlobalVarDecl(static_cast<FE::AST::VarDeclStmt*>(s), m);
            else
                apply(*this, *s, m);
        }
    }

...
namespace FE::AST
{
    void ASTChecker::addError(const Node& node, const std::string& msg)
    {
        std::string loc = " at line " + std::to_string(node.line_num);
        if (node.col_num >= 0) loc += ":" + std::to_string(node.col_num);
        errors.push_back(msg + loc);
    }

    bool ASTChecker::isIntegerType(Type* t)
    {
        if (!t) return false;
        Type_t k = t->getBaseType();
        return k == Type_t::BOOL || k == Type_t::INT || k == Type_t::LL;
    }
    bool ASTChecker::isFloatType(Type* t) { return t && t->getBaseType() == Type_t::FLOAT; }
    bool ASTChecker::isBoolType(Type* t) { return t && t->getBaseType() == Type_t::BOOL; }
    bool ASTChecker::isVoidType(Type* t) { return !t || t->getBaseType() == Type_t::VOID; }

    bool ASTChecker::canConvertTo(Type* from, Type* to)
    {
        if (!from || !to) return false;
        if (from == to) return true;
        if (from->getTypeGroup() == TypeGroup::POINTER || to->getTypeGroup() == TypeGroup::POINTER)
            return from->getTypeGroup() == to->getTypeGroup();
        if (isVoidType(from) || isVoidType(to)) return false;
        return true;
    }

    bool ASTChecker::collectConstValue(const ExprValue& v, Type* expect, VarValue& out)
    {
        if (!expect || !v.isConstexpr) return false;
        switch (expect->getBaseType())
        {
            case Type_t::BOOL: out = VarValue(static_cast<bool>(v.getBool())); return true;
            case Type_t::INT: out = VarValue(static_cast<int>(v.getInt())); return true;
            case Type_t::LL: out = VarValue(static_cast<long long>(v.getLL())); return true;
            case Type_t::FLOAT: out = VarValue(static_cast<float>(v.getFloat())); return true;
    ...
    bool ASTChecker::visit(Root& node)
    {
        ...
        if (!mainExists)
        {
            errors.push_back("Function main not defined");
            res = false;
        }

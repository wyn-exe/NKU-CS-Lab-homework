...
module exe(
    ...
    input      [206:0] ID_EXE_bus_r, // 来自译码级的控制/数据信息
    output             EXE_over,     // 执行级是否完成本条指�??
    output     [190:0] EXE_MEM_bus,  // 向访存级传�?�的总线内容
    ...
    assign EXE_forward_valid = EXE_valid & rf_wen
     ...
     wire [31:0] ov_b = inst_sub_ov ? (~alu_operand2 + 1'b1) : alu_operand2;
     wire [31:0] ov_sum = alu_operand1 + ov_b;
     wire ov_signed = (inst_add_ov | inst_sub_ov) &
                       ((~alu_operand1[31] & ~ov_b[31] & ov_sum[31]) |
                        ( alu_operand1[31] &  ov_b[31] & ~ov_sum[31]));

     wire exc_overflow = EXE_valid & ~exc_valid_i & ov_signed;
     wire exc_valid_exe = exc_valid_i | exc_overflow;
     wire [4:0] exc_code_exe = exc_valid_i ? exc_code_i :
                                exc_overflow ? `EXC_CODE_OV : 5'd0;
     wire [31:0] bad_addr_exe = exc_valid_i ? bad_addr_i : 32'd0;
    ...
    // =========================================================================
     assign EXE_MEM_bus = {mem_control,store_data,
                           ...
                           exc_valid_exe,exc_code_exe,bad_addr_exe,
                           rf_wen,rf_wdest,
                           pc};

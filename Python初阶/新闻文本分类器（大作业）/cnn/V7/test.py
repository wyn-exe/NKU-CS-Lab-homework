...
# 加载预训练的Word2Vec模型
pretrained_word2vec_path = 'cnn/V5/word2vec.model'
...
class TextCNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_filters, filter_sizes, output_dim, dropout, weight_decay=1e-4):
        super().__init__()
...
# 应用数据增强
train_df['content'] = train_df['content'].apply(data_augmentation)
...
for epoch in range(num_epochs):
    ...
    for texts, labels in train_loader:
        ...
        loss = criterion(predictions, labels) + model.regularization_loss()
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

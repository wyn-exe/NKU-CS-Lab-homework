...
int main(int argc, char *argv[])
{
    ...
    for(int i = test_begin; i <= test_end; ++i){
        ...
        // NTT_Iteration_multiply_openmp(a, b, ab, n_, p_);
        // NTT_CRT_4mod_openmp(a, b, ab, n_, p_);
        // ——————以上函数调用时需要把整个代码结构改成LL版本——————
        double start_time = MPI_Wtime();
        // NTT_mpi(a, b, ab, n_, p_); //参数设置为1，4，4
        // NTT_mpi_pthread(a, b, ab, n_, p_);//参数设置为2，2，4
        // NTT_mpi_barrett(a, b, ab, n_, p_);//参数设置为1，4，4
        NTT_mpi_pthread_barrett(a, b, ab, n_, p_);//参数设置为2，2，4
        double end_time = MPI_Wtime();
        double elapsed_time = end_time - start_time;
        // ——————以上函数调用时需要把计时方式改成mpi版本——————
